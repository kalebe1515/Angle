...
// Controlsys 子模块列表 | Controlsys submodules
const submodules = [
  { type: 'pid', label: 'PID控制器' },
  { type: 'state', label: '状态空间' },
  { type: 'input', label: '输入' },
  ...
  const handleRun = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8080/run', {
        ...
        body: JSON.stringify({ module: 'controlsys', params })
      });
