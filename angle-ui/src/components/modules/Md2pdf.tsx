...
// Md2pdf 模块 | Md2pdf module
const Md2pdf: React.FC = () => {
  const [output, setOutput] = useState('');
  const [content, setContent] = useState('# Markdown to PDF\n\nType your markdown content here...');
  ...
  const handleRun = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8080/run', {
        ...
        body: JSON.stringify({ module: 'md2pdf', params: { content } })
      });
  ...
  return (
    <div style={{ padding: 24, color: '#888', background: '#fffde7', borderRadius: 8, margin: 16, boxShadow: '0 2px 8px #f5e9c6' }}>
      <div style={{ fontWeight: 600, fontSize: 16, marginBottom: 8 }}>Markdown 转 PDF | Markdown to PDF</div>
      <textarea
        value={content}
